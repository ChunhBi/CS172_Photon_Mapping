...
std::vector<std::shared_ptr<Geometry>> makeObjMesh(const std::string &path,
                                                   ...
                                                   Float scale,
                                                   vec3 translation, vec3 rotation_axis) {
  ...
  for (auto& v : vertices)
  {
      Eigen::Matrix3f rotation_matrix = Eigen::Quaternionf::FromTwoVectors(vec3(0, 0, 1), rotation_axis).toRotationMatrix();
      v = rotation_matrix * (v * scale) + translation;
  }
  for (auto& n : normals)
  {
      Eigen::Matrix3f rotation_matrix = Eigen::Quaternionf::FromTwoVectors(vec3(0, 0, 1), rotation_axis).toRotationMatrix();
      n = (rotation_matrix * n).normalized();
  }
