...
class Light {
 ...
 public:
   ...
   */
  virtual bool intersect(Interaction &interaction, const Ray &ray) = 0;
...
class AreaLight : public Light {
 ...
 public:
   ...
   * @return whether ray hit the light
   */
  bool intersect(Interaction &interaction, const Ray &ray) override;
...
public :
    explicit PointLight(const vec3& position, const vec3& color);
    vec3 emission(vec3 pos, vec3 dir) override;
    vec3 sample(Interaction& refIt, Float* pdf) override;
    Float pdf(const Interaction& ref_it, vec3 pos) override;
    bool intersect(Interaction& interaction, const Ray& ray) override;
